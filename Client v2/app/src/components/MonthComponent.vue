<template>
    ...
    <TableTemplate :measurements="measurements" :date="date" :device="device" :time="'day'"></TableTemplate>
</template>
...
const getData = (refresh = false) => {
  store?.getters
    .getMonth$(props.device, formatters.formatDate(date.value), refresh)
    .subscribe((data) => { measurements.value = data?.reverse(); });
};

const changeMonth = (count: number) => {
  const newDate = moment(date.value).add(count, 'month');
